...
#warn "converting $type={$value}\n";
    ...
    foreach my $s (split ',', $value) {
        ...
        my $date;
        if (exists $hash->{param}->{TZID}) {
            my $source_tz = $hash->{param}->{TZID};
            $source_tz = $self->{timezone_map}->{$source_tz} if exists $self->{timezone_map}->{$source_tz};

            $s = 'TZID=' . $source_tz . ':' . $s;
        };
        eval {
            #warn "parsing {$s} as datetime using $dfmt";
